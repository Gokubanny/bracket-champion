...
const PublicBracketPage = () => {
  ...
  allMatches
  .filter((m) => m.status === "upcoming" && m.scheduledDate && m.teamA && m.teamB)
  ...
  {tournament.gameFormat && <Badge variant="secondary" className="text-[10px]">{tournament.gameFormat}</Badge>}
  ...
  {showCountdown && <CountdownTimer targetDate={tournament.startDate} className="mt-3" />}
    ...
    <div className="max-w-5xl mx-auto px-4 pb-8 space-y-6">
      <PageBreadcrumbs className="pt-4" items={[{ label: "Tournaments", href: "/tournaments" }, { label: tournament.name }]} />
      ...
      <AnimatePresence>
        {(showChampion || tournament.status === "completed") && (
          ...
          >
            ...
            <h2 className="text-xl font-bold">{champion ? `🏆 ${champion.name} — Champion!` : "Tournament Complete!"}</h2>
          ...
          {liveMatches.map((match) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-sm font-medium truncate">{match.teamA?.name ?? "TBD"} vs {match.teamB?.name ?? "TBD"}</span>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-lg font-bold tabular-nums">{match.scoreA ?? 0} – {match.scoreB ?? 0}</span>
      ...
      {upcomingScheduled.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
            <CalendarClock className="h-3.5 w-3.5" /> Upcoming Fixtures
          </p>
          {upcomingScheduled.slice(0, 6).map((match) => (
            <div key={match.id} className="flex items-center justify-between rounded-lg border border-border/50 bg-muted/20 px-4 py-2.5">
              <span className="text-sm font-medium truncate">{match.teamA?.name ?? "TBD"} vs {match.teamB?.name ?? "TBD"}</span>
              <span className="text-xs text-muted-foreground shrink-0 flex items-center gap-1.5 ml-3">
                <CalendarClock className="h-3 w-3" />
                {format(new Date(match.scheduledDate!), "EEE d MMM, h:mm a")}
              </span>
            </div>
          ))}
        </div>
      ...
      <Tabs defaultValue={isGroupKnockout ? "groups" : "bracket"}>
        <TabsList className={`flex gap-4 w-full ${gridCols}`}>
          ...
          <TabsTrigger value="leaderboard" className="gap-1.5"><BarChart3 className="h-3.5 w-3.5" />Standings</TabsTrigger>
          <TabsTrigger value="scorers" className="gap-1.5"><Star className="h-3.5 w-3.5" />Scorers</TabsTrigger>
        ...
        <TabsContent value="leaderboard" className="mt-4">
          {leaderboardLoading || groupsLoading ? (
            ...
            // During group stage: show per-group standings so teams know where they stand
            <GroupStandings groups={groups} advancingPerGroup={tournament.teamsAdvancingPerGroup} />
